...
MouseArea {
    ...
    readonly property string powerModeIconSrc: !isInDefaultPowerProfile
    ...
    Rectangle {
        ...
        width: Math.max(10, Math.round(root.width * 0.46))
        height: width
        anchors.right: parent.right
        anchors.bottom: parent.bottom
        radius: width / 2
        color: Kirigami.Theme.backgroundColor
        visible: powerModeIcon.visible && root.isManuallyInhibited && !root.isInDefaultPowerProfile
        z: 1

        Kirigami.Icon {
            anchors.fill: parent
            anchors.margins: 1
            source: "system-suspend-inhibited-symbolic"
        }
    }
