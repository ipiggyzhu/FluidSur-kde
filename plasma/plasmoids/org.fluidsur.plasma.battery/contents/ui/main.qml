...
PlasmoidItem {
    ...
    InhibitionControl {
        ...
        onIsManuallyInhibitedChanged: {
            Plasmoid.configuration.manualInhibitionDesired = isManuallyInhibited;
        }
    }
    ...
    onInhibitionChangeRequested: inhibit => {
        Plasmoid.configuration.manualInhibitionDesired = inhibit;
    ...
    Component.onCompleted: {
        ...
        // KDE stores the inhibition cookies in plasmashell memory. Restore them
        // after a shell restart when the user left the switch enabled.
        if (Plasmoid.configuration.manualInhibitionDesired && !inhibitionControl.isManuallyInhibited) {
            inhibitionControl.isSilent = true;
            const reason = i18n("The battery applet has enabled suppressing sleep and screen locking");
            inhibitionControl.inhibit(reason);
        }
    }
